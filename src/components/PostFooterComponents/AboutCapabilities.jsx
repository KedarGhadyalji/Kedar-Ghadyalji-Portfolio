/**
 * What: two stacked home post-footer sections — a boxed "About" intro panel
 * and a flat "What I Do" capability list (one row per discipline).
 * Data from: src/data/home.js (aboutIntro, whatIDo); hero photo from
 * /public/Kedar.png.
 * Used by: src/components/PostFooterHome.jsx. (AboutHomePage.jsx and
 * WhatIDo.jsx in this folder render the same two sections separately but
 * are unused dead code — see the note in each file.)
 */
...
const AboutCapabilities = () => (
  <>
      ...
      >
        <div className="flex flex-col items-start gap-5 sm:flex-row sm:items-center sm:gap-6">
          ...
          <div className="flex-1">
            ...
            <h2 className="text-3xl font-bold leading-tight text-white transition-colors duration-200 group-hover:text-black sm:text-4xl">
              {aboutIntro.title}
            </h2>
    ...
    <section className="mb-10">
      ...
      <h2 className="mb-6 text-3xl font-bold text-white sm:mb-8 sm:text-4xl pixel-shadow">
        {whatIDo.subtitle}
      </h2>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {whatIDo.cards.map((item) => {
          const IconComponent = item.logo; // Dynamically assign the component reference
          return (
            <div
              key={item.title}
            ...
            >
              <div className="mb-4 grid h-12 w-12 place-items-center rounded-xl border border-[#5ce1e6]/30 bg-[#0b0f1f]/60">
                {/* Dynamic Lucide Icon Render instead of <img> */}
                {IconComponent && (
                  <IconComponent className="h-7 w-7 text-[#5ce1e6] group-hover:animate-pulse" />
                )}
              </div>
              <h3 className="font-bold leading-tight text-white text-lg sm:text-xl">
                {item.title}
              </h3>
              <p className="mt-2 font-source-code-pro text-base text-white/70">
                {item.description}
              </p>
            </div>
          );
