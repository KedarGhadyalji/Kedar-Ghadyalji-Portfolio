/**
 * What: the root layout — per-route background/overlay, navbar, footer,
 * the route table, the floating "Kirby" mascot, and the home-only
 * post-footer (cave scene + PostFooterHome). Every route renders inside
 * this shell.
 * Data from: src/data/siteConfig.js (pageConfig for background/overlay per
 * route, pageMeta for <title>/description, `location` for the footer);
 * static assets from ./assets.
 * Used by: src/main.jsx, which mounts <App /> at the document root.
 */
import {
  useRef,
  useEffect,
...
import { kirbyfloating, rocks, grass, japanese_town } from "./assets";
...
const AppContent = () => {
  ...
  useDocumentMeta(pageMeta[location.pathname] ?? NOT_FOUND_META);
  ...
  useLayoutEffect(() => {
    ...
    const onMove = (e) => {
      if (pressed) attract(e.clientX, e.clientY);
    };
    const onUp = () => {
      pressed = false;
    };
    ...
    const frame = (now) => {
      ...
      el.style.left = `${pos.x}px`;
      el.style.top = `${pos.y + window.scrollY}px`;
      el.style.transform = `translate(-50%, -50%) rotate(${rot.toFixed(2)}deg) scale(${sx.toFixed(3)}, ${sy.toFixed(3)})`;
  ...
  return (
    <div className="relative">
      ...
      {showOverlay && (
        <div
          className="absolute inset-0 bg-gradient-to-br from-purple-800 to-teal-400 opacity-50"
          style={{ zIndex: 1 }}
        />
      )}
      ...
      {darken && (
        <div
          ...
          style={{ backgroundColor: `rgba(6,8,18,${darken})`, zIndex: 2 }}
        />
      )}
      ...
      <div className="bg-primary min-h-screen w-full flex flex-col relative">
        ...
        <div
          className={[
            "absolute inset-x-0 top-0 bottom-[120px] sm:bottom-[88px]",
            styles.paddingX,
            "flex flex-col",
          ].join(" ")}
          style={{
        ...
        >
          ...
          <div
            className={[styles.boxWidth, "mb-3 relative"].join(" ")}
            style={{ zIndex: 2 }}
          >
            <Navbar />
          </div>
        ...
        {/* Main Content */}
        <div
          className={["flex-grow flex flex-col relative justify-center"].join(
            " ",
          )}
        >
        ...
        <div className={[styles]}>
          <div ref={postFooterRef}>
            <div
              ...
              style={{
                backgroundImage: `url(${japanese_town})`,
                backgroundSize: "cover",
            ...
            >
              ...
              <div className="relative flex flex-col w-full items-center">
                ...
                <div
                  className="flex justify-center py-6 w-full px-4 sm:px-6 lg:px-8"
                  style={{
                ...
                >
                  <div
                    className={[
                      styles.boxWidth,
                      "flex flex-col items-center z-10 w-full",
                    ].join(" ")}
                  >
                    <div className="w-full flex flex-col sm:flex-row justify-between items-center">
                      <p className="text-white text-sm text-center sm:text-left mb-2 sm:mb-0"></p>
